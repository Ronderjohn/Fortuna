...
def resolve_live_checkpoint_dir(root: Path | str = "models") -> Optional[Path]:
...
class RLSignalGenerator:
            ...
            if (sub / "policy.zip").exists():
                try:
                    ...
                    if (sub / "metadata.json").exists():
                        meta = PolicyCheckpoint.read(sub / "metadata.json")
                        n_bars = meta.n_bars
                    else:
                        meta = self._metadata
                    builder = FeatureBuilder(n_bars=n_bars)
                    if (sub / "normalizer.json").exists():
                        builder.load(sub / "normalizer.json")
    ...
    def predict(
        ...
        # Route to a regime-specific policy if one is active.
        if self._current_regime and self._current_regime in self._regime_policies:
            model, builder, _meta = self._regime_policies[self._current_regime]
        else:
            model, builder = self._model, self._builder
        ...
        return RLLiveSignal(
            ...
            source="rl_policy",
            metadata={
                "run_id": self._metadata.run_id if self._metadata else "unknown",
                "policy_type": self._metadata.policy_type if self._metadata else "unknown",
